...
func binarySearch(search *[]bool, data *[]bool, bufferOverflow int64) []int {
	...
	for ; i < len(*data) && i+advance < len(*data); i++ {
		...
		if matchCount == advance {
			...
			since := int64(i)
			fmt.Print("%d", since+bufferOverflow)
			fmt.Print(" to ")
			//The upper matching border
			until := since + int64(advance)
			fmt.Print("%d", until+bufferOverflow)
			fmt.Print(" , ")
...
func binaryReplace(search *[]bool, data *[]bool, replace *[]bool, bufferOverflow int64) []byte {
	...
	matchedData := binarySearch(search, data, bufferOverflow)
